...
public class JiraRestSession implements JiraSession {
	...
	public void connect(String userName, String password) {
		LOG.debug("Connnecting via REST as : {}", userName);
		try {
			jiraRestClient = jcf.createWithBasicHttpAuthentication(webServiceUrl.toURI(), userName, password);
			LOG.debug("REST Session service endpoint at " + webServiceUrl.toString());
		} catch (URISyntaxException e) {
			throw new IllegalStateException("ServiceException during JiraRestClient contruction", e);
		}
		LOG.debug("Connected");
	}
	...
	public void close() {
		try {
			...
			throw new IllegalStateException("ServiceException during JiraRestClient destruction", e);
		}
	}
